This document is for people who intend to contribute code to the HTTP Addon project. It contains all the details you need to get started contributing to the HTTP Add-on project.

## Getting started

To get started, first [fork](https://github.com/kedacore/http-add-on/fork) this repository to your own GitHub account. 

## Prerequisites

### Go

All of the code in this project is written in the [Go programming language](https://golang.org). You'll need to have the Go tooling installed in your development environment. See [golang.org/dl](https://golang.org/dl) for instructions on getting started.

### Docker (or similar)

You'll need [Docker](https://docker.com) or a similar container platform to build and push container images, so they can be used in your Kubernetes cluster. If you decide to use Docker, see the [Docker installation instructions](https://www.docker.com/get-started) for instructions on how to get started. If not, you'll need tooling that can build standard container images from a `Dockerfile` and push those container images to a standard container registry.
